...
import { Bell, ExternalLink, FolderOpen, LayoutGrid, Pause, Pencil, Play, Plus, ScrollText, Settings, SkipForward, Timer, Trash2, TrendingDown, TrendingUp, X } from "lucide-react";
...
function SpotifyWidget() {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: SPOTIFY_STATUS_QUERY_KEY });
    ...
    <>
      ...
      <div className="flex min-w-0 items-center gap-1.5">
        ...
        <div className="min-w-0 max-w-44 truncate font-mono text-[11px] text-white/60">
          {data.title}
          {data.artist && <span className="text-white/30"> · {data.artist}</span>}
        </div>
      </div>
    </>
  );
...
function SshFolderPicker({
  ...
  const load = useCallback(
    async (target: string) => {
      ...
      setError(null);
      try {
        if (!window.electron) throw new Error("Desktop app required for SSH browsing.");
        ...
        setError(
          /permission denied|publickey|batch mode|host key|resolve|connect|timed out|refused/i.test(raw)
  ...
  return (
    <div className="absolute inset-0 z-10 flex flex-col bg-black/90 backdrop-blur-xl">
      <div className="flex items-center justify-between gap-2 border-b border-white/10 px-3 py-2">
        <span className="truncate text-xs font-medium text-white/70">{host}</span>
        <button
          onClick={onCancel}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {loading && <div className="px-3 py-4 text-center text-xs text-white/30">Loading…</div>}
        {error && !loading && (
        ...
        {!loading && !error && (
          <div className="flex flex-col">
            ...
            {dirs.length === 0 && (
              <div className="px-3 py-4 text-center text-xs text-white/25">No subfolders</div>
            )}
            {dirs.map((d) => (
              <button
                key={d}
...
function ProjectsDropdown() {
  ...
  const saveAdd = () => {
    ...
    createMutation.mutate(
      { client, body: { name: addName.trim(), path: addPath.trim(), ssh_host: addSshHost.trim() || null } },
      { onSuccess: () => { setAddName(""); setAddPath(""); setAddSshHost(""); setShowAdd(false); } },
    );
  };
      ...
      >
              ...
              {editingId === p.id ? (
                <div className="space-y-2 p-3">
                  ...
                  <div className="flex gap-1.5">
                    ...
                    />
                    <button
                      onClick={() => browsePath(editSshHost, editPath, (path, name) => { setEditPath(path); if (!editName) setEditName(name); })}
          ...
          {showAdd ? (
            <div className="space-y-2 p-3">
              ...
              <div className="flex gap-1.5">
                ...
                />
                <button
                  onClick={() => browsePath(addSshHost, addPath, (path, name) => { setAddPath(path); if (!addName) setAddName(name); })}
                  className="shrink-0 rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-xs text-white/50 transition-colors hover:bg-white/10 hover:text-white/80"
                >
                  Browse
                </button>
        ...
        {sshBrowse && (
          <SshFolderPicker
            ...
            onSelect={(p) => { sshBrowse.apply(p); setSshBrowse(null); }}
          />
        )}
      </PopoverContent>
    </Popover>
  ...
  return (
    <>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          <button
            onClick={() => { addWorkspace(); setOpen(false); }}
            ...
            <div className="max-h-48 overflow-y-auto border-t border-white/[0.06]">
              {templates.map((t) => (
                ...
                >
                  ...
                  <span className="truncate text-[11px] text-white/35">{widgetSummary(t.widgets)}</span>
                </button>
              ))}
            </div>
          )}
      ...
      <WorkspaceTemplatesDialog open={manageOpen} onOpenChange={setManageOpen} />
    </>
  );
}
