...
def upgrade() -> None:
  ...
  op.execute("UPDATE notifications SET link = body, body = NULL WHERE body LIKE 'http%'")


def downgrade() -> None:
  # Restore URL links back to body on downgrade
  op.execute("UPDATE notifications SET body = link WHERE link IS NOT NULL AND body IS NULL")
