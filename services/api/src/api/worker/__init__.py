...
class MaxRetriesMiddleware(dramatiq.Middleware):
  ...
  def before_process_message(self, broker: dramatiq.Broker, message: dramatiq.MessageProxy) -> None:
    ...
    max_retries = message.options.get("max_retries", actor.options.get("max_retries", settings.WORKER_MAX_RETRIES))
...
class SchedulerMiddleware(dramatiq.Middleware):
  ...
  def __init__(self) -> None:
    ...
    self.startup_jobs: list[Callable[..., Any]] = []

  @property
  def actor_options(self) -> set[str]:
    return {"cron_trigger", "run_on_startup"}

  def after_declare_actor(self, broker: dramatiq.Broker, actor: dramatiq.Actor[Any, Any]) -> None:
    if cron_trigger := actor.options.get("cron_trigger"):
      self.cron_triggers.append((actor.send, cron_trigger))
    if actor.options.get("run_on_startup"):
      self.startup_jobs.append(actor.send)
...
class LogContextMiddleware(dramatiq.Middleware):
  ...
  def before_process_message(self, broker: dramatiq.Broker, message: dramatiq.MessageProxy) -> None:
    structlog.contextvars.bind_contextvars(actor_name=message.actor_name, message_id=message.message_id)
  ...
  **options: Any,
  ...
  def decorator(
    ...
    async def _wrapped_fn(*args: P.args, **kwargs: P.kwargs) -> R:
      async with JobQueueManager.open(dramatiq.get_broker(), RedisMiddleware.get()):
